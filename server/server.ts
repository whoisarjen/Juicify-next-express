...
app.post('/auth/login', (req: any, res: any) => require('./mongoDB/auth/login')(req, res));

app.post('/find/:where', (req: any, res: any) => require(`./mongoDB/find/${req.params.where}`)(req, res)); // Need function to cache find things etc.

app.post('/guest/:where', async (req: any, res: any) => {
...
app.post('/:what/:where', async (req: any, res: any) => {
    await verifyToken(req)
    await verifyIDS(req)
    await require(`./mongoDB/${req.params.what}/${req.params.where}`)(req, res)
        .then(async (data: any) => await handleSynchronization(req, res, data, 'change'))
        .catch((err: any) => {
            console.log(err)
            res.status(404).send({ error: 'Wrong query' })
        })
...
const handleSynchronization = async (req: any, res: any, data: any, whatToDo: any) => {
...
const createSynchronizationObject = async (user_ID: any) => {
...
const updateSynchronizationObject = async (user_ID: any, where: any) => {
