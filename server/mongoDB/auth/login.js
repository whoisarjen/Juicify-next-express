      ...
      bcrypt.compare(req.body.password, user.password, (err, respo) => {
        ...
        Model.find({ user_ID: user._id })
          .then(async function (workout_plan) {
            ...
            Model.find({
              $and: [{ user_ID: user._id }, { deleted: { $exists: false } }],
            })
                      ...
                      .then(async function (daily_measurement) {
                        if (daily_measurement.length > 0) {
                          for (let i = 0; i < daily_measurement.length; i++) {
                            if (
                              daily_measurement[i].nutrition_diary &&
                              daily_measurement[i].nutrition_diary.length > 0
                            ) {
                              for (
                                let a = 0;
                                a < daily_measurement[i].nutrition_diary.length;
                                a++
                              ) {
                                if (
                                  !daily_measurement[i].nutrition_diary[a]
                                    .calories
                                )
                                  daily_measurement[i].nutrition_diary[a] =
                                    await loadProduct(
                            ...
                            if (
                              daily_measurement[i].workout_result &&
                              daily_measurement[i].workout_result.length > 0
                            ) {
                              for (
                                let a = 0;
                                a < daily_measurement[i].workout_result.length;
                                a++
                              ) {
                                for (
                                  let b = 0;
                                  b <
                                  daily_measurement[i].workout_result[a].results
                                    .length;
                                  b++
                                ) {
                                  daily_measurement[i].workout_result[
                                    a
                                  ].results[b] = await loadExercise(
                                    daily_measurement[i].workout_result[a]
                                      .results[b]
                                  );
                                }
                              }
                        ...
                        Model.find({ user_ID: user._id })
                          .then(async function (favourite_product) {
                            if (favourite_product.length > 0) {
                              for (
                                let i = 0;
                                i < favourite_product.length;
                                i++
                              ) {
                                favourite_product[i] = await loadProduct(
                                  favourite_product[i]
                                );
                              }
                            }
