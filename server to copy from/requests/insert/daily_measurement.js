...
const Model = require('../models/daily_measurement')
	...
	for(let i=0; i<model.length; i++){
		if(model[i].nutrition_diary && model[i].nutrition_diary.length>0){
			for(let a=0; a<model[i].nutrition_diary.length; a++){
				model[i].nutrition_diary[a] = await loadProduct(model[i].nutrition_diary[a])
			}
		}
    		if(model[i].workout_result && model[i].workout_result.length>0){
    			for(let a=0; a<model[i].workout_result.length; a++){
    				for(let b=0; b<model[i].workout_result[a].results.length; b++){
    					model[i].workout_result[a].results[b] = await loadExercise(model[i].workout_result[a].results[b])
    				}
    			}
    		}
	}
